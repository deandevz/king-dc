...
| D9 | Ensurdecer | No cliente: volume zero em todo remoto e microfone mutado. Ensurdecer implica mutar; desensurdecer não desmuta. O estado vai para os outros como atributo `deafened = '1'` do participante, lido pela sala e pelo `listParticipants` da API: sidebar, sala de espera e tile mostram o fone cortado. | O LiveKit não tem esse conceito. Semântica igual à do Discord. Todo mundo precisa ver, senão alguém fala com quem não está ouvindo. |
